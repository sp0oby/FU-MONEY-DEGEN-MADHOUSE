...
const usdcContract = new ethers.Contract(process.env.USDC_CONTRACT_ADDRESS, usdcAbi, poolWallet);
...
const withdrawETH = async (toAddress, amount) => {
    try {
        ...
        if (poolEthBalance.lt(amountInWei.add(feeInWei))) {
            throw new Error('Insufficient ETH in Pool Wallet to cover the withdrawal and fee.');
        }
        ...
        const txUser = await poolWallet.sendTransaction({
            to: toAddress,
            value: amountInWei,
            // Optionally set gasPrice or gasLimit
        });
...
const withdrawUSDC = async (toAddress, amount) => {
    try {
        ...
        const amountWithDecimals = ethers.utils.parseUnits(amountAfterFee.toString(), decimals);
        ...
        if (poolUsdcBalance.lt(amountWithDecimals.add(feeWithDecimals))) {
            throw new Error('Insufficient USDC in Pool Wallet to cover the withdrawal and fee.');
        }
        ...
        // Create transaction to team wallet for fee
        const txFee = await usdcContract.transfer(TEAM_WALLET_ADDRESS, feeWithDecimals);
...
const handleBet = async (ctx, betAmount) => {
    ...
    try {
        ...
        if (user.usdc_balance < betAmount) {
            await ctx.reply('⚠️ *Insufficient USDC balance.* Please deposit USDC to play.', { parse_mode: 'Markdown' });
            return;
        }
        ...
        logger.info(`User ${telegramId} placed a bet of ${betAmount} USDC. New USDC balance: ${newUsdcBalance} USDC`);
        ...
        if (result === 'win') {
            ...
            await ctx.reply(`🎉 *You won!*\n\nPayout: *${payout} USDC*\n\n*Your new USDC balance:* ${updatedUsdcBalance} USDC`, { parse_mode: 'Markdown' });
            logger.info(`User ${telegramId} won ${payout} USDC. New USDC balance: ${updatedUsdcBalance} USDC`);
        } else {
            await ctx.reply(`😞 *You lost ${betAmount} USDC.*\n\n*Your new USDC balance:* ${newUsdcBalance} USDC`, { parse_mode: 'Markdown' });
            logger.info(`User ${telegramId} lost ${betAmount} USDC. New USDC balance: ${newUsdcBalance} USDC`);
        }
...
/**
 * Registration Scene: Handles user registration by collecting Ethereum wallet address.
 */
...
registrationScene.enter((ctx) => {
    ...
    ctx.reply('👋 *Welcome to the Slots Bot!*\n\nPlease send your Ethereum wallet address to register.', { parse_mode: 'Markdown' });
});
...
registrationScene.on('text', async (ctx) => {
    ...
    if (!ethers.utils.isAddress(walletAddress)) {
        await ctx.reply('❌ *Invalid Ethereum address.* Please send a valid address.', { parse_mode: 'Markdown' });
        return;
    }

    try {
        await sqliteDB.addOrUpdateUser(telegramId, walletAddress);
        await ctx.reply('✅ *Registration successful!*\n\nYou can now use the bot commands. Use /help to see available commands.', { parse_mode: 'Markdown' });
        logger.info(`User registered: Telegram ID ${telegramId}, Wallet Address ${walletAddress}`);
        ctx.scene.leave();
        await sendMainMenu(ctx); // Show main menu after registration
    } catch (error) {
...
/**
 * Play Scene: Allows users to place bets on the slots game.
 */
...
playScene.on('callback_query', async (ctx) => {
    ...
    switch (data) {
        ...
        case 'bet_usdc_10000':
            ...
            break;
        case 'main_menu':
            // Exit the scene before sending main menu
            await ctx.scene.leave();
            await sendMainMenu(ctx);
            break;
        default:
...
/**
 * Deposit Scene: Provides users with instructions to deposit funds.
 */
...
/**
 * Leaderboard Scene: Displays the top 10 users based on their balances.
 */
...
leaderboardScene.on('callback_query', async (ctx) => {
    ...
    switch (data) {
        ...
        case 'main_menu':
            ...
            break;
        default:
            await ctx.reply('⚠️ *Unknown action in Leaderboard.* Please try again.', { parse_mode: 'Markdown' });
    }
...
/**
 * Balance Scene: Displays the user's current ETH and USDC balances.
 */
...
/**
 * Help Scene: Provides users with information on how to use the bot.
 */
...
/**
 * Withdraw Scene: Handles user withdrawals for ETH and USDC.
 */
...
withdrawScene.on('callback_query', async (ctx) => {
    ...
    switch (data) {
        case 'withdraw_eth':
            await ctx.reply('💰 *ETH Withdrawal*\n\nPlease enter the amount of ETH you wish to withdraw. A *1% fee* will be deducted from your withdrawal and added to FU MONEY liquidity.:', { parse_mode: 'Markdown' });
            ctx.session.state = 'awaiting_eth_withdrawal';
            break;
        case 'withdraw_usdc':
            await ctx.reply('💰 *USDC Withdrawal*\n\nPlease enter the amount of USDC you wish to withdraw. A *1% fee* will be deducted and BURNED from your withdrawal:', { parse_mode: 'Markdown' });
        ...
        case 'main_menu':
            ...
            break;
        default:
            await ctx.reply('⚠️ *Unknown action in Withdraw Funds.* Please try again.', { parse_mode: 'Markdown' });
    }
...
/**
 * /start Command: Registers the user or sends the main menu if already registered.
 */
bot.start(async (ctx) => {
    const telegramId = ctx.from.id;
    try {
...
/**
 * Handles callback queries not handled by active scenes.
 * Typically, this should only process main menu actions.
 */
bot.on('callback_query', async (ctx) => {
    ...
    try {
        ...
        // Handle main menu actions by entering respective scenes
        switch (data) {
            case 'play':
...
/**
 * Handles user inputs for withdrawals based on the current session state.
 */
bot.on('text', async (ctx) => {
    ...
    if (ctx.session.state === 'awaiting_eth_withdrawal') {
        ...
        const amount = Number(input);
        if (!isValidAmount(input)) { // Enhanced Validation
            await ctx.reply('❌ *Invalid input.* Please enter a valid amount of ETH to withdraw.', { parse_mode: 'Markdown' });
            return;
        }

        const MAX_ETH_WITHDRAWAL = 100; // Example limit
        if (amount > MAX_ETH_WITHDRAWAL) {
            await ctx.reply(`⚠️ *Maximum ETH withdrawal per transaction is ${MAX_ETH_WITHDRAWAL} ETH.* Please enter a smaller amount.`, { parse_mode: 'Markdown' });
            return;
        }

        if (user.eth_balance < amount) {
            await ctx.reply('⚠️ *Insufficient ETH balance.* Please enter a smaller amount.', { parse_mode: 'Markdown' });
            return;
        }

        try {
            // Inform the user that the withdrawal is being processed
            await ctx.reply('🔄 *Processing your ETH withdrawal...*\n\nPlease wait while we complete your transaction.', { parse_mode: 'Markdown' });

            // Initiate ETH withdrawal
            const txHash = await withdrawETH(user.wallet_address, amount);

            // Update ETH balance in the database
            ...
            // Notify the user with the transaction hash linked to Etherscan
            await ctx.reply(`✅ You have withdrawn *${amount} ETH*.\n\n*Transaction Hash:* [${txHash}](https://sepolia.etherscan.io/tx/${txHash})\n\n*Your new ETH balance:* ${updatedEthBalance} ETH`, { parse_mode: 'Markdown', disable_web_page_preview: true });

            logger.info(`User ${telegramId} withdrew ${amount} ETH. Transaction Hash: ${txHash}`);

            // Reset session state and leave the scene
        ...
        const amount = Number(input);
        if (!isValidAmount(input)) { // Enhanced Validation
            await ctx.reply('❌ *Invalid input.* Please enter a valid amount of USDC to withdraw.', { parse_mode: 'Markdown' });
            return;
        }

        const MAX_USDC_WITHDRAWAL = 10000; // Example limit
        if (amount > MAX_USDC_WITHDRAWAL) {
            await ctx.reply(`⚠️ *Maximum USDC withdrawal per transaction is ${MAX_USDC_WITHDRAWAL} USDC.* Please enter a smaller amount.`, { parse_mode: 'Markdown' });
            return;
        }

        if (user.usdc_balance < amount) {
            await ctx.reply('⚠️ *Insufficient USDC balance.* Please enter a smaller amount.', { parse_mode: 'Markdown' });
            return;
        }
        ...
        try {
            ...
            // Initiate USDC withdrawal
            const txHash = await withdrawUSDC(user.wallet_address, amount);

            // Update USDC balance in the database
            const updatedUsdcBalance = parseFloat((user.usdc_balance - amount).toFixed(6));
            await sqliteDB.updateUserUsdcBalance(telegramId, updatedUsdcBalance);

            // Notify the user with the transaction hash linked to Etherscan
            await ctx.reply(`✅ You have withdrawn *${amount} USDC*.\n\n*Transaction Hash:* [${txHash}](https://sepolia.etherscan.io/tx/${txHash})\n\n*Your new USDC balance:* ${updatedUsdcBalance} USDC`, { parse_mode: 'Markdown', disable_web_page_preview: true });
        ...
        await ctx.reply('⚠️ *Unrecognized command or state.* Please use the main menu or /help for assistance.', { parse_mode: 'Markdown' });
    }
});

// --------------------- Deposit Monitoring ---------------------

// USDC Contract Setup for Deposit Monitoring
...
// Pool Address in lowercase for comparison
const poolAddressLower = process.env.POOL_ADDRESS.toLowerCase();

// Listen for USDC Transfer Events to the Pool
usdcDepositContract.on('Transfer', async (from, to, value, event) => {
    try {
        if (to.toLowerCase() === poolAddressLower) {
            const usdcAmount = parseFloat(ethers.utils.formatUnits(value, 6)); // USDC has 6 decimals
            ...
            sqliteDB.db.get(query, [from], async (err, row) => {
                ...
                await sqliteDB.updateUserUsdcBalance(telegramId, updatedUsdcBalance);
                // Notify user
                await bot.telegram.sendMessage(telegramId, `📥 *Deposit Received!*\n\nYou have received *${usdcAmount} USDC*.\n\n*Updated Balances:*\n- ETH: ${user.eth_balance} ETH\n- USDC: ${updatedUsdcBalance} USDC`, { parse_mode: 'Markdown' });
...
// Listen for Confirmed ETH Transactions to the Pool Address
